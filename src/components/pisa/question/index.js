...
import {ExcContext, PisaExcContext} from "helpers/contexts";
...
const ViewExc = ({onChangeCompletedComponent, questionComponent = {}, setAnswers}) => {
    ...
    const {disabledExc, isView,isCheck = false} = useContext(ExcContext)
    ...
    }, [variants, input, questionComponent,isCheck])
    ...
    useDebounce(() => {
        ...
        const filteredChecked = variants.filter(item => item.checked)[0]



        const data = {
            block_id: questionComponent.id,
            type: questionComponent?.variants?.typeVariants,
            answer: questionComponent?.variants?.typeVariants === "select" ?  {
                id: filteredChecked.id
            } : input
        }

        request(`${BackUrl}pisa/student/complete/pisa/test/${testId}`, "POST", JSON.stringify(data), headers())
            .then(res => {
                console.log("hello", res)
            })
    },1000,[variants, input, questionComponent,isChanged,isCheck])
...
const CreateExc = ({questionComponent, onSetCompletedComponent, onDeleteComponent, extra}) => {
    ...
    const onSubmit = () => {
        ...
        if (variants && !isEmpty) {
            ...
            request(`${BackUrl}pisa/block/questions/${questionComponent?.id}`,method,formData,headersImg())
                .then(res => {
