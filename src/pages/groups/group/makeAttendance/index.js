...
import LoaderWithOverlay from "components/ui/loaderWithOverlay/LoaderWithOverlay";
...
const MakeAttendanceIndex = () => {
    ...
    const onCheck = async (type, id) => {
        ...
        const checkScores = user.score.filter(item => !item.activeBall > 0)
        ...
        if (type === "yes") {
            if (checkScores.length > 0) {
                const newScores = user.score?.map(s => {
            ...
            const defaultedScores = user.score.map(sc => ({...sc, isNotChecked: false, activeBall: 0}))
        ...
        await setUsers(items => items.map(item => {
            if (item.id === id) {
                return newUser
            }
            return item
