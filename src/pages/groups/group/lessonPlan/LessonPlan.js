...
import {BackUrlForDoc, headersOldToken, PlatformUrlApi} from "constants/global";
...
const LessonPlan = ({backBtn}) => {
    ...
    useEffect(() => {
        if (groupData.platform_id) {
            request(`${PlatformUrlApi}lesson_plan_list/${groupData.platform_id}`, "GET", null, headersOldToken() )
                .then(res => {
    ...
    useEffect(() => {
        if (year && month ) {
            request(`${PlatformUrlApi}lesson_plan_list/${groupData.platform_id}/${year}-${month}`, "GET", null, headersOldToken() )
                .then(res => {
    ...
    },[groupData.platform_id, month, request, year])
    ...
    useEffect(() => {
        const data = {
            ...
            day,
            year,
            group_id: groupData.platform_id
        }


        if (year && month && day && groupData?.platform_id ) {
            request(`${PlatformUrlApi}get_lesson_plan`,"POST",JSON.stringify(data),headersOldToken() )
                .then(res => {
    ...
    const onSubmit = (data) => {
        ...
        request(`${PlatformUrlApi}change_lesson_plan/${planId}`,"POST",JSON.stringify({...data,students}), headersOldToken())
            .then(res => {
                const alert = {
                    active : true,
                    message: res.msg,
