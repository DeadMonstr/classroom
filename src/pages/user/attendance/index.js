...
const AttendanceStudent = () => {
	...
	useEffect(() => {
		...
		request(`${BackUrl}student/combined_attendances2/`,"GET",null,headers())
			.then(res => {
				setDays(res.data.dates)
				setAttendance(res.data.attendances)
			})

		request(`${BackUrl}student/student_group_dates/`,"GET",null,headers())
			.then(res => {
	...
	useEffect(() => {
		if (year && month) {
			...
			request(`${BackUrl}student/combined_attendances2/`,"POST",JSON.stringify(data),headers())
				.then(res => {
					setDays(res.data.dates)
					setAttendance(res.data.attendances)
				})
