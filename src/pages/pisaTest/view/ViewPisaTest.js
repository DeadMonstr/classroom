...
import cls from "./viewPisaTest.module.sass"
...
const ViewPisaTest = () => {
    ...
    const onSubmit = () => {
        if (rightExc.length > 0 && rightDisabled) return
    ...
    return (
        <div className={cls.createPisaTest}>
            ...
            <div className={cls.submit}>
                <Button disabled={((leftExc.length > 0 && leftDisabled) || (rightExc.length > 0 && rightDisabled)) } onClick={onSubmit} form={"createLesson"} type={"submit"}>
                    Tasdiqlash
                </Button>
            </div>
...
const CreateExcPisa = ({typeSide, onChangeExc, pisaId, oldExc = [], setDisabled}) => {
    ...
    useEffect(() => {
        if (answers.length > 0 && setDisabled) {
            ...
            <div className={cls.createLesson}>
                ...
                >
                    <div className={cls.createLesson__container}>
                        <PisaExcContext.Provider value={{disabledExc:false, isView: true}}>
                            {renderedComponents}
