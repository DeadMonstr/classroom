...
const ObserveTeacherLessonIndex = () => {
    ...
    useEffect(() => {
        request(`${PlatformUrlApi}observe_info`,"GET",null,headersOldToken())
            .then(res => {
    ...
    const onSubmit = (data) => {
        ...
        request(`${PlatformUrlApi}teacher_observe/${id}`, "POST", JSON.stringify({list: fields,month,day}),headersOldToken())
            .then(res => {
                const alert = {
                    active : true,
                    message: res.msg,
